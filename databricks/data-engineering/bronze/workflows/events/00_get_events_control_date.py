...
if df.count() > 0:
    ...
    gdelt_events_last_modified = df.filter(df.TABLE_NAME == 'gdelt_events').select('LAST_UPDATE_DATE').collect()[0][0]
    gdelt_gkg_last_modified = df.filter(df.TABLE_NAME == 'gdelt_gkg').select('LAST_UPDATE_DATE').collect()[0][0]

    # Suma un día para obtener la próxima fecha a procesar
    next_date_to_process_events = (gdelt_events_last_modified + timedelta(1)).strftime('%Y-%m-%d')
    ...
    print(f"Última fecha procesada tabla events: {gdelt_events_last_modified}")
    print(f"Próxima fecha a procesar events:     {next_date_to_process_events}")
