...
FINAL_NEWS_SUMMARY = (gkg_scraped
.filter(col("Date") >= '2024-08-15')
.filter(col("AverageTone") < 0)
.withColumn("NewsWithTINFA", when(col("THEMES").like("%TRANSPORT_INFRASTRUCTURE%"),1)) # FLAG THAT THE NEWS HAS THE THEME TRANSPORT INFRASTRCTURE
.withColumn("NewsWithTRADE", when(col("THEMES").like("%TRADE%"),1)) # FLAG THAT THE NEWS HAS THE THEME TRADE 
...
.withColumn("Total", col("NewsWithTINFA")+col("NewsWithTRADE")+col("NewsWithME") + col("NewsWithPS") + col("NewsWithMI"))
.withColumn('Rank', row_number().over(Window.partitionBy("Date").orderBy(col("Total").desc())))
.join(SPARK_GKG_NEWS_TPR.drop("contenturl"), ['Date','Rank','Summary'],'left')
.join(SPARK_GKG_NEWS_TPR, ['Date','Rank','Summary'],'left')
...
.select('DATE', 'EXTRACTION_DATE', 'COUNTRYCODE', 'LOCATIONCODE', 'AVERAGETONE', 'IS_RUTA_TRANSPACIFICA', 'IS_RUTA_TRANSATLANTICA', 'IS_RUTA_DEL_CABO_DE_BUENA_ESPERANZA', 'SOURCEURLS', 'summarized_url')

# COMMAND ----------

table_name = "gold.g_gdelt_gkg_weights_detail_report"
delta_table_path = f"s3://databricks-workspace-stack-e63e7-bucket/unity-catalog/2600119076103476/gold/gdelt/gkg_weights_detail_report/"  # O especifica una ruta personalizada si usas S3 o DBFS
