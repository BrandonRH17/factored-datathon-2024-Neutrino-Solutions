...
# MAGIC %md
# MAGIC # Models Generation to Predict Status of USCA port 
# MAGIC
# MAGIC In this notebook you can find the training process for the model for USCA port status predictions. 
...
# MAGIC
# MAGIC 4. Filter for locations of interest, In our case as we are focusing on the port on USCA, we would focus on this locations: USCA
...
.filter(col("LocationCode").isin("USCA")) # FILTER THE NEWS RELATED TO THE LOCATIONS OF THE PORT (CHECK DEFINITION IN THE CELL ABOVE FOR MORE DETAILS IN THE CODES)
)
...
# MAGIC %md
# MAGIC The P value in our series is 0.1305777662672269. Is not lower than 0.05. this means that our series in NOT STATIONARY. So we have to make the serie Stationary to have Better Forecasting Results
...
# MAGIC %md
# MAGIC Now, are P value is   4.905306332675798e-13, Very below to 0.05. So now we can start thinking abour Time Series Forecasting
...
my_callback = MyThresholdCallback(threshold=0.1360) # CREATING CALL BACK
...
model.save("USCA_model.keras")

# COMMAND ----------

scaler_filename = "scalerUSCA.save"
joblib.dump(scaler, scaler_filename) 

# COMMAND ----------

scaler_filename = "scalerUSCA_Obj.save"
...
# MAGIC %md
# MAGIC It's pretty good! Now this is the end of the model generating model for the USCA Port
