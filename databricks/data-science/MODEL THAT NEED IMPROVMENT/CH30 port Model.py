...
# MAGIC %md
# MAGIC # Models Generation to Predict CH30 port status
# MAGIC
# MAGIC In this notebook you can find the training process for the model for CH30 port status predictions. 
...
# MAGIC
# MAGIC 4. Filter for locations of interest, In our case as we are focusing on the port CH30, we would focus on this locations: "CH30"
...
.filter(col("LocationCode").isin("CH30")) # FILTER THE NEWS RELATED TO THE LOCATIONS OF THE PORT (CHECK DEFINITION IN THE CELL ABOVE FOR MORE DETAILS IN THE CODES)
)
...
# MAGIC %md
# MAGIC This could be a problem It seam's that there is no direct autocorrelation
...
# MAGIC %md
# MAGIC The P value in our series is  0.00025088906796925887. Is lower than 0.05. this means that our series is STATIONARY. We are going to apply the same differencing techinique to keep all the process the same for all ports
...
# MAGIC %md
# MAGIC Now, are P value is    8.52152678446897e-27, Very below to 0.05. So now we can start thinking abour Time Series Forecasting
...
my_callback = MyThresholdCallback(threshold=0.138) # CREATING CALL BACK
...
model.save("CH23_model.keras")

# COMMAND ----------

scaler_filename = "CH23TPR.save"
joblib.dump(scaler, scaler_filename) 

# COMMAND ----------

scaler_filename = "CH23TPR_Obj.save"
...
# MAGIC %md
# MAGIC It's pretty good! Now this is the end of the model generating model for the CH23 Model
