...
# MAGIC %md
# MAGIC # Models Generation to Predict Status of CH02 Port Route
# MAGIC
# MAGIC In this notebook you can find the training process for the model for CH02 port status predictions. 
...
# MAGIC
# MAGIC 4. Filter for locations of interest, In our case as we are focusing on the transpacific route, we would focus on this locations: "CH02"
...
.filter(col("LocationCode").isin("CH02")) # FILTER THE NEWS RELATED TO THE LOCATIONS OF THE PORT (CHECK DEFINITION IN THE CELL ABOVE FOR MORE DETAILS IN THE CODES)
)
...
# MAGIC %md
# MAGIC By analyzing this graph we are going to use a lag of 7, even if this correlation plot might be a problem
...
# MAGIC %md
# MAGIC The P value in our series is 1.1986178838328178e-06. Is  lower than 0.05. this means that our series in  STATIONARY. So we have to make the serie Stationary using differencing to keep the same methods applied.
...
# MAGIC %md
# MAGIC Now, are P value is   8.94732566386417e-30, Very below to 0.05. So now we can start thinking abour Time Series Forecasting
...
# MAGIC 4. In our process of finding the best architecture. We use one regularization method that is a drop out layer with a probability of 0.2
...
my_callback = MyThresholdCallback(threshold=0.08) # CREATING CALL BACK
...
model.save("CH02_model.keras")

# COMMAND ----------

scaler_filename = "scalerCH02.save"
joblib.dump(scaler, scaler_filename) 

# COMMAND ----------

scaler_filename = "scalerCH02_Obj.save"
...
# MAGIC %md
# MAGIC It's pretty good! Now this is the end of the model generating model for the CH02
