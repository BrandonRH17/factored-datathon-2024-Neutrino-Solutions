...
# MAGIC %md
# MAGIC # Models Generation to Predict Status of CA10 Port
# MAGIC
# MAGIC In this notebook you can find the training process for the model for CA10 Port status predictions. 
...
# MAGIC
# MAGIC 4. Filter for locations of interest, In our case as we are focusing on the CA10 port, we would focus on this location: "CA10"
...
.filter(col("LocationCode").isin("CA10")) # FILTER THE NEWS RELATED TO THE LOCATIONS OF THE PORT (CHECK DEFINITION IN THE CELL ABOVE FOR MORE DETAILS IN THE CODES)
)
...
# MAGIC %md
# MAGIC The P value in our series is  0.014382804760275472. Is lower than 0.05. this means that our series in STATIONARY. In this case we don't need to do apply techinques for stationarity. But we are going to apply the difference to mantain the same techniques for all the models
...
# MAGIC %md
# MAGIC Now, are P value is    2.2572800018362713e-16, Very below to 0.05. So now we can start thinking abour Time Series Forecasting
...
my_callback = MyThresholdCallback(threshold=0.1270) # CREATING CALL BACK
...
model.save("CA10_model.keras")

# COMMAND ----------

scaler_filename = "scalerCA10.save"
...
# MAGIC %md
# MAGIC It's pretty good! Now this is the end of the model generating model for the CA10
